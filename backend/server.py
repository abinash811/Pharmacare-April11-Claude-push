...
async def receive_goods(
    purchase_id: str,
    receive_data: ReceiveGoodsInput,
    current_user: User = Depends(get_current_user)
):
    """
    Receive goods against a purchase (partial or full)
    Creates/merges batches and creates stock movements
    """
    # Get purchase
    purchase = await db.purchases.find_one({"id": purchase_id}, {"_id": 0})
    if not purchase:
    ...
    if purchase['status'] == 'cancelled':
        raise HTTPException(status_code=400, detail="Cannot receive goods for cancelled purchase")
    
    if purchase['status'] == 'closed':
        raise HTTPException(status_code=400, detail="Purchase is already closed")
    ...
    for receive_item in receive_data.items:
        ...
        for item in purchase['items']:
            if item['id'] == receive_item.purchase_item_id:
                purchase_item = item
                break
        
        if not purchase_item:
            raise HTTPException(status_code=404, detail=f"Purchase item {receive_item.purchase_item_id} not found")
        
        # Check if receiving more than ordered
        already_received = purchase_item.get('received_qty_units', 0)
        outstanding = purchase_item['qty_units'] - already_received
        
        if receive_item.qty_units > outstanding:
            raise HTTPException(
                status_code=400,
                detail=f"Cannot receive {receive_item.qty_units} units for {purchase_item['product_name']}. Only {outstanding} units outstanding."
            )
        
        # Get product for units_per_pack
        product = await db.products.find_one({"sku": purchase_item['product_sku']}, {"_id": 0})
        if not product:
            raise HTTPException(status_code=404, detail=f"Product {purchase_item['product_sku']} not found")
        
        units_per_pack = product.get('units_per_pack', 1)
        
        # Use provided prices or fall back to purchase item prices
        cost_price = receive_item.cost_price_per_unit if receive_item.cost_price_per_unit else purchase_item['cost_price_per_unit']
        mrp = receive_item.mrp_per_unit if receive_item.mrp_per_unit else purchase_item['mrp_per_unit']
        
        # Convert expiry date
        expiry_date = None
        if receive_item.expiry_date:
            expiry_date = datetime.fromisoformat(receive_item.expiry_date)
        
        # Check if batch exists (for merging)
        existing_batch = await db.stock_batches.find_one({
            "product_sku": purchase_item['product_sku'],
            "batch_no": receive_item.batch_no,
            "location": "default"
        }, {"_id": 0})
        
        batch_id = None
        qty_packs = receive_item.qty_units / units_per_pack
        
        if existing_batch:
            # Merge with existing batch
            batch_id = existing_batch['id']
            new_qty = existing_batch['qty_on_hand'] + qty_packs
            ...
            logger.info(f"Merged batch {receive_item.batch_no}: added {qty_packs} packs, new total: {new_qty}")
        else:
            # Create new batch
            ...
            await db.stock_batches.insert_one(new_batch)
            logger.info(f"Created new batch {receive_item.batch_no} with {qty_packs} packs ({receive_item.qty_units} units)")
        ...
        movement = {
            ...
            "batch_no": receive_item.batch_no,
            "qty_delta_units": receive_item.qty_units,
            "movement_type": "purchase",
            "ref_type": "purchase",
            "ref_id": purchase_id,
            "location": "default",
            "reason": f"Goods received from {purchase['supplier_name']} - Purchase {purchase['purchase_number']}",
            "performed_by": current_user.id,
        ...
        receipt_item = GoodsReceiptItem(
            ...
            qty_units=receive_item.qty_units,
            cost_price_per_unit=cost_price,
            mrp_per_unit=mrp
        )
        ...
        batch_updates.append({
            "item_id": receive_item.purchase_item_id,
            "new_received_qty": already_received + receive_item.qty_units
        })
    
    # Update purchase items with received quantities
    for update in batch_updates:
        await db.purchases.update_one(
            {"id": purchase_id, "items.id": update['item_id']},
            {"$set": {"items.$.received_qty_units": update['new_received_qty']}}
        )
    ...
    receipt = GoodsReceipt(
        ...
        receipt_date=datetime.fromisoformat(receive_data.receipt_date),
        received_by=current_user.id,
        received_by_name=current_user.name,
        supplier_invoice_no=receive_data.supplier_invoice_no,
        note=receive_data.note,
        items=receipt_items
    )
    
    receipt_doc = receipt.model_dump()
    receipt_doc['receipt_date'] = receipt_doc['receipt_date'].isoformat()
    ...
    await db.goods_receipts.insert_one(receipt_doc)
    ...
    for item in updated_purchase['items']:
        received = item.get('received_qty_units', 0)
        ordered = item['qty_units']
    ...
    await db.audit_logs.insert_one({
        ...
        "new_value": {
            ...
            "status": new_status,
            "items_received": len(receipt_items)
        },
        "performed_by": current_user.id,
        "performed_by_name": current_user.name,
        "reason": f"Goods receipt created - {len(receipt_items)} items",
        "created_at": datetime.now(timezone.utc).isoformat()
    })
    ...
    return {
        ...
        "receipt_id": receipt.id,
        "status": new_status,
        "batches_created": len([r for r in receipt_items if not existing_batch]),
        "batches_updated": len([r for r in receipt_items if existing_batch]),
...
async def update_purchase(
    ...
    """Update purchase (only in draft status)"""
    purchase = await db.purchases.find_one({"id": purchase_id}, {"_id": 0})
    if not purchase:
        raise HTTPException(status_code=404, detail="Purchase not found")
    ...
    if purchase_data.supplier_id:
        ...
        if not supplier:
            raise HTTPException(status_code=404, detail="Supplier not found")
        update_dict['supplier_id'] = purchase_data.supplier_id
        update_dict['supplier_name'] = supplier['name']
    
    if purchase_data.purchase_date:
        update_dict['purchase_date'] = purchase_data.purchase_date
    
    if purchase_data.supplier_invoice_no:
        update_dict['supplier_invoice_no'] = purchase_data.supplier_invoice_no
    
    if purchase_data.supplier_invoice_date:
        update_dict['supplier_invoice_date'] = purchase_data.supplier_invoice_date
    
    if purchase_data.note:
        update_dict['note'] = purchase_data.note
    ...
    if purchase_data.items:
        ...
        update_dict['tax_value'] = tax_value
        update_dict['round_off'] = round_off
...
@api_router.post("/purchases/{purchase_id}/close")
async def close_purchase(
    purchase_id: str,
    current_user: User = Depends(get_current_user)
):
    """Mark purchase as closed (manually, regardless of received status)"""
    purchase = await db.purchases.find_one({"id": purchase_id}, {"_id": 0})
    if not purchase:
        raise HTTPException(status_code=404, detail="Purchase not found")
    
    if purchase['status'] == 'cancelled':
        raise HTTPException(status_code=400, detail="Cannot close cancelled purchase")
    ...
    return {"message": "Purchase closed successfully"}

@api_router.post("/purchases/{purchase_id}/cancel")
async def cancel_purchase(
    purchase_id: str,
    current_user: User = Depends(get_current_user)
):
    """Cancel purchase (only if no goods received)"""
    purchase = await db.purchases.find_one({"id": purchase_id}, {"_id": 0})
    if not purchase:
        raise HTTPException(status_code=404, detail="Purchase not found")
    ...
    for item in purchase['items']:
        if item.get('received_qty_units', 0) > 0:
            raise HTTPException(
                status_code=400,
                detail="Cannot cancel purchase with received goods. Create a purchase return instead."
            )
...
@api_router.get("/purchases/{purchase_id}/receipts", response_model=List[GoodsReceipt])
async def get_purchase_receipts(
    purchase_id: str,
    current_user: User = Depends(get_current_user)
):
    """Get all goods receipts for a purchase"""
