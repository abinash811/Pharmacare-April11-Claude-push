# PharmaCare — Roadmap
# Version: 2.36 | Last updated: September 11, 2026
# Type: Living Status
...
| As a pharmacist entering a distributor's bill for the first time, I want to add a new distributor or a new medicine right there in New Purchase if it isn't in my system yet, instead of being blocked until I go add it in Suppliers/Inventory first. | ❌ **Not built** — found Aug 25, 2026 | Confirmed via code: `SupplierDropdown.jsx`/`PurchaseItemsTable.jsx` only search existing `Supplier`/`Product` records fetched at page load — no create-inline path exists. Same root need as the CSV/Excel bill-import gap above (both are ways new SKUs enter via the purchase, not a separate catalog step first) but distinct: this covers manual entry too, not just file import. P0 — this blocks the single most basic real-world use case (a genuinely new bill), see the RULE MISSES LOG entry same date. |
