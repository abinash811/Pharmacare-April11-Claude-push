# PharmaCare — Roadmap
# Version: 3.20 | Last updated: September 22, 2026
# Type: Living Status
...
| Receipt & Print settings — Print sub-tab | 🐛 **Real gaps found — full review Sep 22, 2026, live-tested (Sep 14 fix confirmed still holding)** | The Sep 14, 2026 header bug (below) is confirmed still fixed — live-verified again with a fresh zero-data pharmacy. But this pass (direct request: "review it thoroughly, check each individual feature") found the underlying reason that fix didn't fully close the gap: **there are three independent print/receipt code paths for the same bill, and they read different subsets of print settings.** (1) In-session **"Save & Print"** at bill creation (`BillingWorkspace/components/PrintReceipt.jsx`) reads `bill_header`/`bill_footer`/`print_patient_name`/`print_signature` correctly. **Correction, same day:** an earlier version of this row claimed GSTIN/Drug License/FSSAI/PAN were shown unconditionally on this path — that was wrong, based on grepping only inside `PrintReceipt.jsx` itself and missing its actual call site. `BillingWorkspace/utils/buildPrintPharmacyInfo.js` (called from `BillingWorkspace/index.jsx`) already gates all four fields correctly (`gstin: p.print_gstin ? g.gstin : ''`, same pattern for `drug_license`/`fssai`/`pan`) before they ever reach `PrintReceipt.jsx` as props — verified by reading the real call chain, not assumed. This is not a bug; no fix needed for these four toggles. What IS still a real, confirmed gap on this path: it has no GST-summary table at all, so `print_gst_summary` has nothing to gate here even though Tax & GST's own docs (`docs/21_FEATURES.md`) describe it as printing on bills. (2) **`BillDetail`'s "Print"** (`window.print()`) and (3) the **backend `GET /bills/{id}/pdf`** (`routers/billing.py`) both correctly honor all six toggles + header/footer — but both are hardcoded to A4 (`from reportlab.lib.pagesizes import A4` / `pagesize=A4` in the PDF generator; `BillDetail/index.jsx` never reads `paper_size` at all) — so a pharmacy configured for 80mm/58mm thermal gets a full A4 page every time they reprint or download any past bill, even though the *original* Save & Print correctly used the thermal layout. Net effect: the exact same bill can print differently depending on which of the three buttons is used, and a pharmacist has no way to get a correctly-thermal-formatted **reprint** of a past bill at all. Live-verified end-to-end: zero-data pharmacy, real Settings values (`REVIEW-HEADER-MARKER`/`REVIEW-FOOTER-MARKER`, 80mm paper size), real bill, real `GET /bills/{id}/pdf` download inspected — header/footer correct, page format A4 regardless. Not fixed this pass — reported to Abinash as findings, awaiting build decision. **Phase: v1** — reprinting a bill is a routine counter task, not an edge case; the three-paths-disagree shape is the actual root cause the Sep 14 fix only partially addressed. |
