# PharmaCare — Roadmap
# Version: 2.78 | Last updated: September 14, 2026
# Type: Living Status
...
| Receipt & Print settings — Print sub-tab | ✅ | Live bill preview, paper size (A4/A5/58mm/80mm), show/hide toggles, header/footer — real end-to-end, consumed by the PDF generator |
...
**Settings — reviewed Sep 14, 2026 (second product-review pass, first to include the competitor-benchmark step — see `docs/01_PRODUCT.md` §10's new Settings competitor notes).** No further crash-level or decorative-toggle bugs found beyond the Sep 13 pass and the three rows above — all three flagged gaps were put to Abinash directly and parked by his own call, not silently skipped. Zero-data browser walkthrough **not done this pass** — `playwright`/`chrome-devtools` MCP tools were disconnected (connection timeout) for the whole session; backend field-verification (GET/PUT round-trip via grep of `settings.py` against `PharmacySettings`) was done instead and found no further gaps. Re-run the live walkthrough once browser tools are available again.
