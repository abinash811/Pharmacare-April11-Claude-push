...
| Returns settings (window, require original bill, partial returns) | ✅ **Fixed Sep 13, 2026** | Same bug shape as Billing: `GET` hardcoded `{"return_window_days": 7, "require_original_bill": False, "allow_partial_return": True}` regardless of DB state; `PUT` never read a `"returns"` key. Now real columns, enforced in `create_sales_return`: `return_window_days` rejects a return against a bill older than the window; `allow_partial_return` (when false) requires every item on the original bill to be returned in full. `require_original_bill` is now dormant — see the row below. |
| Manual (no-original-bill) sales returns | 🚫 **Removed Sep 23, 2026, direct product decision** | Built Sep 15, 2026 (nullable `original_bill_id`, `allow_manual_returns` permission, `require_original_bill` Settings toggle), removed the same session it was discussed: nothing tied a manual return's quantity or refund amount to an actual prior sale — a real fraud/leakage surface. `POST /sales-returns` now unconditionally 400s without `original_bill_id`, for every role, regardless of the (now-dormant) `require_original_bill` setting. `ManualItemSearch.tsx` deleted. Schema/settings left in place (nullable columns, dormant toggle) rather than migrated out. Removing this exposed that `create_sales_return` had **no other permission check at all** — `allow_manual_returns` was the only one, scoped only to the branch just deleted, so the ordinary bill-linked path has never been role-gated for any role. Same class of gap as Billing's 4 money endpoints, same standing decision: `# permission-exempt:`, left as-is, revisit post-launch. |
| Sales Return — inline bill picker | ✅ **Built Sep 23, 2026** | Same-day direct follow-up: with manual returns gone, the only way in was leaving Sales Returns for Billing, finding the bill, opening it, clicking Return — a real detour for something that should be one click. `SalesReturnCreate` now shows an inline search (`BillPicker.tsx`) when no `billId` is set, reusing `GET /bills`'s existing `search` param (no new endpoint). Only completed sales are listed (`status` not `draft`/`parked`, filtered client-side) — a **due** bill stays pickable on purpose, since the due-balance-credit UX built Sep 15, 2026 has no other entry point. Picking a result just sets `?billId=` on the same page, reusing 100% of the existing fetch/items/save logic. `SalesReturnsList`'s "New Return" goes straight to `/billing/returns/new` now that this exists. |
| Sales Return quantity cap was per-request, not cumulative | ✅ **Fixed Sep 23, 2026** | Direct question ("can a user return more than they sold") led to checking, not assuming: `create_sales_return` capped a new return's quantity against the original bill's quantity only, never against quantity already returned in an earlier, separate return on the same bill — two separate returns could each claim the full original quantity and both would be accepted. `update_sales_return` (financial edit) had **no** quantity validation at all. Both now compute `max_returnable = original_qty - already_returned_across_every_prior_return_on_this_bill` (matched by `batch_number`), same pattern `purchase_returns.py`'s `create_purchase_return` already used (`already_returned_qty`/`max_returnable_qty`) — sales_returns never got the equivalent. 5 new regression tests (`test_sales_return_quantity_cap.py`), confirmed 3 of 5 fail against the pre-fix code via `git stash`, pass after. |
| A return doesn't adjust the bill total; Dashboard/Sales report/Margin report don't net returns out | 🔵 **Reviewed Sep 23, 2026 — kept as-is, not a bug** | Walked through with a concrete example (₹1,000 bill, ₹500 returned) and confirmed: the Billing list, the printed/reprinted bill, `BillORM.grand_total_paise` itself, Dashboard's sales totals, the Sales report, and the Margin report are all unaffected by a return — each sums/reads the original bill only. Direct decision: **keep it this way** — "bills and returns should tally," each a fixed historical record read side by side, not netted into one number. Full reasoning in `docs/07_BUSINESS_LOGIC.md`'s Sales Returns section. Not covered by this decision, already correct: the **GST report** already nets a return's GST out of the period's output tax (a compliance requirement, checked and confirmed unaffected), and **Inventory** already restores real stock on a return (also checked, also unaffected). |
| Dashboard — Sales (Month) summary card + `month_returns` was always 0 | ✅ **Built/Fixed Sep 23, 2026** | Direct follow-up to the row above: asked for a small Sales/Returns/Net table on the Dashboard so the relationship is visible in one place without changing the gross-everywhere decision. Building it surfaced a real, separate bug: `quick_stats.month_returns` (and `daily_trend`'s per-day `"returns"`) were computed from `Bill.invoice_type == "SALES_RETURN"` — a condition no real code path has ever set (sales returns are their own resource, `routers/sales_returns.py`, never a Bill row) — so this figure was always 0 for every real pharmacy, no matter how many returns actually existed. Fixed `get_dashboard_analytics` to sum real `SalesReturn` rows (same shape the GST report already used correctly); added `month_sales`/`net_sales` to `quick_stats`. New `SalesReturnsSummaryCard.tsx` replaces the old standalone "Returns (Month)" tile — a per-instruction table, not the 3-separate-tiles pattern Purchases uses (Purchases deliberately left unchanged, by direct instruction, so the two sections now look different on purpose). 2 new backend tests + 1 updated frontend test, confirmed to fail against the pre-fix code. |
| Purchases — Pack/Unit entry on the items table | ✅ **Built Sep 24, 2026** | Real product-clarity gap named directly by Abinash: a pharmacist buying 10 strips of 10 tablets at ₹30/strip had to type Qty=100, PTR=₹3.00 — the backend was always correct (real per-unit storage), only the entry screen forced the pack→unit math onto the pharmacist. Pack/Unit toggle per line (`PurchaseItemsTable.jsx`), defaults to Pack for any product with `units_per_pack` > 1, converts to real units in `buildPurchasePayload.js` right before the unchanged `POST/PUT /purchases` call — no backend/schema change. `GET /purchases/{id}` now also returns `units_per_pack` per item (was stored, never returned) so a reloaded draft's toggle has what it needs. Also corrected UC-P16 in `docs/23`, which had wrongly claimed this was backend-built and cited a test name that doesn't exist. 4 new frontend test files + 1 new backend test, all confirmed passing. |
| Purchases — short/excess supply (UC-P18) | ✅ **Built Sep 25, 2026** | Grepped the real code before building this one, per the Sep 24 RULE MISSES LOG (unlike UC-P09/UC-P31 the same day, this gap was confirmed genuinely real). `quantity_ordered`/`quantity_received` already existed as separate `PurchaseItem` columns, but the confirm path always forced them equal — no way to record a short or excess delivery. `received_qty_units` (optional, `None` = no discrepancy) added to `PurchaseItemCreate`; a "Received a different quantity?" link reveals the field per line only when needed, with the variance ("Short by 5"/"Excess by 5") shown live and again on Purchase Detail. Drives real stock added and fixes `PurchaseItem.quantity_received`, a dead field previously always 0 regardless of what confirmed. `qty_units` alone still drives cost/GST/what's owed the supplier — a genuinely short delivery's credit-note conversation is a separate, not-built feature. No schema change. 6 new backend tests + 11 new frontend tests, all confirmed passing. |
| Purchases — price-change warning (UC-P20) | ✅ **Built Sep 25, 2026** | Half of `docs/23`'s claim was wrong, checked before building: "MRP below cost" (`costExceedsMrp`) already existed. Genuinely missing: no price-history comparison anywhere. New `GET /purchases/last-purchase-price` (advisory, same shape as `check-duplicate-invoice`) returns the most recent CONFIRMED purchase's cost/MRP for a product across **any** supplier — direct product decision, a different supplier's rate is still the real signal. Fetched once per line item on add, patched in async, never blocks entry. Warns "Was ₹X last time" on a cost **increase** only; "MRP was ₹X" on **any** MRP change, either direction. `PurchaseItemsTable.jsx` extracted into a new `PurchaseItemRow.jsx` (300-line cap) while wiring this in — third per-line feature added to that row in two days (Pack/Unit, short/excess, now this). 5 new backend tests + 9 new frontend tests (UI + hook), all confirmed passing. |
| Purchases — payment report (UC-P37) | ✅ **Built Sep 25, 2026** | Verified real before building — payments were only ever queryable one purchase at a time (`GET /purchases/{id}/payments`), nothing aggregated them. New `GET /reports/purchase-payments` (same pattern as sibling `GET /reports/purchase-returns`): date-range filtered, `summary` (count + total) plus a `by_method` breakdown, excludes reversed payments. New "Purchase Payments" tab on the Reports page — reuses the page's existing generic CSV/Excel export unchanged (any flat `data` array), so P39's export gap is now partly closed too. Also caught and corrected a stale claim while verifying: `docs/23`'s P35 note about drafts leaking into supplier totals was already fixed (`edfe2f8`), not still open. 5 new backend tests + 3 new frontend tests, all confirmed passing. |
| Billing — "Card" payment method split into Credit Card / Debit Card | ✅ **Built Sep 24, 2026** | Direct follow-up to a Billing use-case walkthrough: `PAYMENT_METHOD.CARD`/`.CREDIT` were a stale, ambiguous pair — `CREDIT` dated from when a bill could carry a running due balance (removed Sep 19, 2026), so it no longer meant anything but "paid by credit card," and neither constant was even imported anywhere (`BillingSubbar.jsx`/`SplitPaymentPanel.tsx` hardcoded their own raw `'card'` strings instead). Replaced with explicit `credit_card`/`debit_card` in `domainConstants.js` (both components now import from it, closing that magic-string gap in the same change); `_VALID_SPLIT_METHODS` (billing.py) updated to match. `StatusBadge`, `PrintReceipt`, and Day-End Closing's `BreakdownTables` all keep rendering the legacy `card`/`credit` values for bills created before this change (Manifesto rule 6 — never rewritten), just no longer offer them as a selectable option. |
...
|------|---------------|----------------------|--------------|
| Sep 24, 2026 | Rule 14 ("no assumptions, verify every time") — same-day repeat of the GST/stock-adjust miss above, this time nearly causing real damage: told Abinash "no way to correct a confirmed purchase" (quoting `docs/23`'s UC-P09 "❌ Missing" row) and started **building a second, conflicting correction mechanism inside `update_purchase`** — new helper functions, a same-day/payment/return/stock-moved lock design, an `AskUserQuestion` round with Abinash — before grepping the router and discovering `PUT /purchases/{id}/correct` (`correct_confirmed_purchase`) already existed: complete, admin-gated, reason-required, audit-logged, wired to `PurchaseDetail`'s "Correct Purchase" menu action, and covered by 6 passing tests. UC-P31 ("Edit or reverse payment — ❌ Missing entirely") was the identical story — also fully built (`PaymentHistorySection.tsx`, `TestPaymentReversal`, 5 passing tests). The doc's own "Batch 4 — correction mechanisms" section near the bottom *already* said both shipped Sep 13, 2026 — only the earlier per-UC detail rows were never updated to match, an internal contradiction inside the same file. | Execution gap, not a tooling gap: `docs/23` was read once at the start of this sub-task and trusted, not re-verified against a real grep for the feature before proposing a design or asking Abinash to decide on one. The two earlier same-day doc-drift fixes (GST report, stock-adjust permission) were each caught by grepping the code directly — this one skipped that step because the doc's prose ("No reversal, no adjustment... permanent, forever") read as unambiguous, which is exactly the trap: a confident-sounding stale claim is more dangerous than a hedged one. | Reverted the in-progress duplicate code before it was ever committed (`git checkout` on the one touched file — nothing else was at risk). Corrected UC-P09, UC-P31, and the stale cross-reference at line 246 in `docs/23` to match the doc's own already-correct Batch 4 note, live-reverified by running both test classes (11 tests, all passing) and checking the frontend wiring directly. Built `scripts/check_doc_status_claims.py` per rule 0's "2nd+ occurrence, gate is mandatory" — **tested it against the actual stale UC-P09/UC-P31 text and confirmed it would NOT have caught either one**: both claims named zero routes in backticks, and the script only fires when a "Missing"/"Broken" row's own text quotes a route that turns out to exist. Kept the script anyway (real, if narrow, value for the rows that do quote something), but the honest primary fix is the one-line addendum added to CLAUDE.md rule 14 itself: grep the router before trusting or building from a "Missing"/"Broken" doc claim, every time — this specific bug shape is a discipline problem a script can't fully mechanize, same conclusion the Sep 22 Margin-report and Sep 19 GST/FLOW-8 entries already reached before a script was even attempted here. |
| Sep 24, 2026 | Rule 14 ("no assumptions, verify every time") — **3rd occurrence of the same-fact-in-two-places class** (see the Sep 22 Margin report and Sep 19 GST/FLOW-8 entries below, both already logged "no automated gate proposed"). Told Abinash "the GST report currently hard-crashes" and "Purchases has real gaps," both quoting stale one-liners (`CLAUDE.md`'s docs index, `docs/24`'s old GST01/GST07 appendix rows, `docs/23`'s permission matrix) instead of checking real code — the GST crash and credit-sale bugs were fixed Sep 12, the stock-adjust permission gap Sep 13; only the Purchases missing-feature list (UC-P09/16/18/20/31, missing reports, return gaps) held up under live re-verification. | Per rule 0: this is now a 3rd occurrence with "no gate" logged each time, which is itself the failure this log's rule 0 exists to catch — a real mechanical gate (diffing doc status glyphs against fix dates across the whole `docs/` tree, not just the file being edited) hasn't been scoped or asked about yet, rather than being judged infeasible. | Live-reverified against real code/API calls (registered a pharmacy, created a bill, called `GET /reports/gst` directly, read `batches.py:475`), corrected all 4 stale spots in place with dated fix notes, bumped every affected doc's version. **Not yet done:** actually proposing/building the cross-doc consistency gate rule 0 requires — flagged to Abinash as a separate decision, not silently skipped a 3rd time. |
| Sep 23, 2026 | `docs/07_BUSINESS_LOGIC.md`'s own rewrite note ("v1.0 described a `POST /api/invoices` flow ... that does not exist anywhere in the real system") wasn't checked against every consumer of the old design it replaced — `get_dashboard_analytics`'s `quick_stats.month_returns` (and `daily_trend`'s per-day `"returns"`) still checked `Bill.invoice_type == "SALES_RETURN"`, the exact literal the same doc already says no real code path has ever set, since Sales Returns became its own resource (`routers/sales_returns.py`). This figure was always 0 for every real pharmacy, silently, for as long as that endpoint has existed | Found only because a direct request ("add a Sales/Returns/Net table") required tracing where `month_returns` actually came from before building on top of it — not from a review of the endpoint itself. The condition never raised an error (a Bill's `invoice_type` is just never equal to that string, so the branch simply never ran), and no test asserted an exact non-zero value for `month_returns` against real return data — the existing test only checked the field was present, which a permanently-0 value still satisfies. Same root-cause shape as the manual-returns removal's stale-reference miss two entries below, and the exact case rule 0 of this log warns about: a design doc's own "this doesn't exist" note should have been grepped against every remaining literal match of the old string, not just the ones already found. | Rewrote the computation to sum real `SalesReturn` rows (same query shape the GST report already used correctly, which never had this bug). 2 new regression tests, confirmed to fail against the pre-fix code (`KeyError` on the newly-added fields, and would have failed on `month_returns == 0` too had a value-asserting test existed before). No automated gate proposed — a lint rule can't know a query condition is semantically dead without executing it against real data, which is exactly what the new test now does. |
| Sep 23, 2026 | Manifesto rule 11 (cross-cutting: business-logic checks, not just endpoint invariants, need the same "is this the whole surface area" review) — `create_sales_return`'s quantity-cap check compared a new return only against the original bill's quantity, never against quantity already returned in an earlier, separate return on the same bill; `update_sales_return` had no quantity check at all | Direct question from Abinash ("can a user return more than they sold") after the inline bill-picker build, not from a review — nobody had asked this specific question of this specific endpoint before. Once asked, checking (not assuming) meant reading `purchase_returns.py`'s sibling `create_purchase_return`, which already solves the identical problem correctly (`already_returned_qty`/`max_returnable_qty`) — proving this wasn't a hard problem, just one sales_returns.py never got asked. No test existed that created two returns against the same bill, so a wrong answer here would have shipped silently. | Ported the same `already_returned`/`max_returnable` pattern to both `create_sales_return` and `update_sales_return` (the latter needed it added from scratch, not just fixed). 5 new regression tests, confirmed 3 of 5 fail against the pre-fix code via `git stash`. No automated gate proposed — this is a business-logic correctness question (does a check account for every prior write to the same aggregate), not a shape-of-code pattern a static check can verify; the real defense is the regression test suite itself, now covering it. |
| Sep 23, 2026 | `docs/08_ARCHITECTURE.md`'s "Standing endpoint invariants" (every mutating endpoint permission-checked) — `POST /sales-returns` had exactly one permission check (`allow_manual_returns`), scoped only to the no-bill branch; the ordinary bill-linked path (the vast majority of real returns) has never been role-gated, for any role, since this endpoint was written | Direct product decision to remove manual returns entirely surfaced this by accident — deleting the `allow_manual_returns` check and re-running `scripts/check_permission_coverage.py` is what caught it, not a review of the endpoint as a whole. The automated gate only checks that *some* call exists somewhere in the function body; it can't tell that the one call present only ever ran on one of two branches, so this passed the gate silently for months. | Same standing decision already made for Billing's 4 money endpoints (docs/15_ROADMAP.md, Sep 22, 2026 entry): marked `# permission-exempt:`, left as-is, not fixed now — real usage after launch should drive who gets restricted from what. No automated gate proposed beyond what exists: `check_permission_coverage.py` already does the only thing a static check realistically can (confirm a call exists); verifying that call actually covers *every* code path, not just one branch, would need real branch-coverage analysis — out of proportion to this being a second, not yet third, occurrence of "the gate proves presence, not correctness" (see the Sep 22 `Decimal`/`entity_type` entries in this same log for the first two). |
| Sep 22, 2026 | Manifesto rule 14 ("no assumptions, verify every time") — a fabricated-data pattern recurring a second time in the same area of the app: `BillingTable.jsx`'s bill-line sub-text used `item.cost_price \|\| item.unit_price * 0.7` as its cost fallback, inventing a 70%-of-MRP "cost" and a matching margin % whenever the real cost was missing, then displaying both as if real | This file's own comment two lines above the removed block (line 70-71, still in the file) names the exact same class of bug already fixed once — a fabricated `prev_mrp` field removed Aug 23, 2026 — without noticing its neighbor had reintroduced the same pattern under a different name. No test asserted on the sub-line's content at all, so a wrong number rendered with zero failures. Reported directly with a screenshot, not caught by review. | Removed the whole sub-line (batch no./cost/margin/composition) rather than just fixing the fallback — it was also redundant with the row's own Batch column and exposed real cost/margin to anyone viewing Billing, a business-sensitivity issue independent of the fabrication bug. 1 new regression test, confirmed to fail against the pre-fix code. No automated gate proposed — a lint rule can't distinguish a legitimate business multiplier from a fabricated placeholder; the real fix is what this entry itself is: naming the pattern in this log so it's recognized on sight next time, per rule 0's instruction to watch for a 2nd+ occurrence. |
| Sep 22, 2026 | `pharmacare-design` skill's own Step 5 ("Sign-in/sign-up forms: correct `autocomplete=\"email\"` / `autocomplete=\"new-password\"` etc. attributes" — a named, already-vetted pattern for exactly this page) | Reported directly by Abinash with a screenshot, not caught by any test or review — none exists that renders the real register form and asserts on `autoComplete`, and a missing HTML attribute produces no visible error in dev, only a browser-specific autofill misfire that varies by what's saved in that browser's own profile. `AuthPage.js` predates the skill's Step 5 note and nothing had gone back to apply it once written. | Added the correct `autoComplete` token to all 11 fields across login and both register steps (see Known Issues row same date for the full list). 3 new regression tests (`AuthPage.autofill.test.tsx`) assert every field's token directly, confirmed to fail against the pre-fix code via `git stash`. No new automated gate proposed — a lint rule could plausibly require `autoComplete` on every `<Input>` in a page under `AuthPage`/`Settings`-shaped forms, but this is the first occurrence of this specific class; revisit if it recurs. |
| Sep 22, 2026 | Manifesto rule 14 ("no assumptions, verify every time") — this Known Issues table itself said "Margin report UI WIP | Low | Data available via API" while the Reports status table two sections up already said the same feature was "✅ Fixed Sep 12, 2026 (Batch 6)" — a contradiction inside the same doc, and the wrong half is what got restated to Abinash on the product-status punch list that led to this fix | Execution gap: the Sep 12, 2026 fix updated the Reports status table but never removed the matching, now-stale Known Issues row it made obsolete — the exact "fixed the code, missed a place the fact is restated" shape logged repeatedly in this table already (see Sep 19 GST/FLOW-8 entry). Nothing forced a cross-check between the two tables. Caught only because building "the fix" meant reading the real endpoint and frontend first per `pharmacare-backend-build`/`pharmacare-frontend-build`, which showed a fully working item-wise table, tab, and 3 passing `test_margin_report.py` tests already in place — not by anyone flagging the doc directly. | Corrected the stale Known Issues row. The one real gap the stale row accidentally still pointed at: `GET /reports/margin`'s `by_category` rollup was computed and returned but never rendered — added `MarginCategoryTable` to close that, not a from-scratch build. No automated gate proposed — same class as prior same-fact-in-two-places misses: a lint rule can't verify two prose table rows describing the same feature agree; the real fix is treating a stale-status flag (Manifesto rule 14) as reason enough to grep the real code before restating either table's claim as current. |
| Sep 22, 2026 | Manifesto rule 16 ("every loading state is a skeleton") applied at the route level, not just inside a page — `App.js` had zero `React.lazy`/`Suspense`/`import()` anywhere, so every authenticated page (Billing, Purchases, Reports, Settings, Team, etc.) was bundled into the same initial JS chunk the login page needed before it could render at all | Asked directly ("why login page loads slower") after the question surfaced during an unrelated product-status review; confirmed by grep (`React.lazy\|Suspense\|import(` — zero matches) rather than assumed. No prior session had checked route-level bundling at all — every other performance/skeleton fix in this log was page-internal (a spinner or blank screen inside an already-loaded page), a different bug shape from the whole app shipping as one bundle regardless of which page loads first. | Converted every route reachable only after login to `React.lazy(() => import(...))`, kept `AuthPage`/`ForgotPassword`/`ResetPassword` as static imports (the actual pre-login critical path), wrapped the authenticated `<Routes>` in `<Suspense fallback={<PageSkeleton />}>` — a skeleton, never a bare spinner, per rule 16 applied to the Suspense boundary itself. Verified via `npx craco build`: previously one bundle carrying the whole app; now a 221.96 kB gzipped main chunk plus ~30 on-demand chunks. No automated gate proposed — a lint rule could plausibly flag a page-level `import` reachable only from an authenticated route and require `lazy()` instead, but no prior instance of this bug class exists yet to justify building it now; revisit if a new heavy page ships as a static import again. |
| Sep 22, 2026 | Manifesto rule 14 ("no assumptions, verify every time") — wiring `_record_audit` into `inventory.py`'s `update_product` produced a real 500 the moment a real GST% edit was tested | Direct instruction to close the known Inventory/Batches audit-log gap. `ProductORM.gst_rate`/`discount_percent` are declared `Mapped[float]` but are actually `Numeric(5,2)` columns — SQLAlchemy returns a genuine `Decimal` at runtime, not a `float`, regardless of the type hint. Copying suppliers.py's old/new-value diffing pattern verbatim (`old_value = getattr(product, col)`) captured that raw `Decimal` straight into the `old_values` dict meant for a JSONB column — Postgres's JSON encoder has no default serialization for `Decimal`. Caught immediately by running the new regression test against the real isolated backend before considering the work done, not by a code read — the test's own traceback named the exact type mismatch. | Cast `Decimal` → `float()` before adding it to `old_values`/`new_values` in `update_product`. `_product_response()` already did this exact same cast for the same two columns (found while root-causing) — the diffing code just hadn't been checked against it. No automated gate proposed — same class as prior field-shape misses in this log: a lint rule can't verify a `Mapped[float]` type hint disagrees with the column's real runtime type without executing the code, which is exactly what the regression test now does on every run. |
| Sep 22, 2026 | Manifesto rule 9 ("no magic strings") + rule 11 (cross-cutting: a new audit entity type has a real frontend consumer) — the new `batches.py` audit entries were first written with `entity_type="batch"` | Copied the shape of `entity_type="product"` (a correct, already-consistent value) without checking what the *existing* stock-batch convention already was elsewhere in the app. `frontend/src/pages/AuditLogBadges.tsx`'s `ENTITY_LABELS`/`ENTITY_TYPES` — the Audit Log page's own filter dropdown and label lookup — only recognize `"stock_batch"`; `"batch"` was never a real value anywhere in the codebase. Caught by checking the frontend consumer directly (grepping `AuditLogBadges.tsx`) before calling the backend work done, not by a test — the backend itself has no schema constraining `entity_type` to a known set, so a wrong string would have inserted successfully and simply never matched the "Stock Batch" filter, silently. | Renamed both `_record_audit` calls in `batches.py` (and the matching test assertions) from `"batch"` to `"stock_batch"`. No automated gate proposed — same reasoning as the Aug 23/25 field-mismatch entries: a lint rule can't verify a new backend string value maps to a real frontend enum without a shared contract neither side has; the real fix is checking the actual consumer before calling a new entity/status/action string done, the same habit "HOW TO BUILD" already asks for on routes and fields. |
...
| Margin report UI WIP | ✅ **Fixed Sep 22, 2026** | Stale row — the item-wise table, tab, and endpoint were already done Sep 12, 2026 (Batch 6, see Reports table above); what was actually missing was the `by_category` rollup the backend already computed and returned but the frontend silently discarded. Added a category-rollup table (`MarginCategoryTable`, `ReportTables.jsx`) above the item table, 2 new jest tests (`ReportTables.margin.test.tsx`). |
| Login page loads slower than it should | ✅ **Fixed Sep 22, 2026** | Confirmed via grep that `App.js` had zero `React.lazy`/`Suspense`/`import()` usage — every page in the app (Billing, Purchases, Reports, Settings, etc.) was bundled into the same initial chunk the login page needed before it could render. Converted every route reachable only after login to `React.lazy`, wrapped the authenticated `<Routes>` in `<Suspense fallback={<PageSkeleton />}>` (skeleton, not a spinner — Manifesto rule 16). Verified via `npx craco build`: previously one monolithic bundle, now a 221.96 kB gzipped main chunk plus ~30 lazy chunks loaded on demand. |
| Register form's "Your pharmacy" step autofilled Pharmacy Name/Address with the signed-in browser's own saved name/email | ✅ **Fixed Sep 22, 2026** | Reported directly with a screenshot: Chrome filled "Pharmacy Name" with the user's own saved name and "Address" with their own saved email. Root cause: not one input on `AuthPage.js` (login or either register step) declared an `autoComplete` attribute at all, so Chrome's own heuristic guessing was the only signal, and it guessed wrong on a two-step form. | Added the correct token to every field: `email`/`current-password` (login), `name`/`email`/`new-password`/`tel` (register step 1), `organization`/`address-line1`/`address-level2`/`address-level1`/`postal-code` (register step 2 — pharmacy name, address, city, state, pincode), `off` on Drug License (no standard token exists). 3 new regression tests (`AuthPage.autofill.test.tsx`), confirmed to fail against the pre-fix code via `git stash`, pass after. |
| Billing's zero-medicine-match message didn't link to Purchases/Inventory, and every bill line showed the batch's real cost price + a margin % (fabricated via an `item.unit_price * 0.7` fallback when cost was missing) to whoever's looking at the screen | ✅ **Fixed Sep 22, 2026** | Reported directly with screenshots. (1) `AddMedicineSearchRow.tsx`'s "No medicine found" message named Purchases/Inventory as bold text, not real navigation — now real `<Link>`s to `ROUTES.PURCHASES.CREATE`/`ROUTES.INVENTORY.LIST`. (2) `BillingTable.jsx` showed batch no. + cost price + margin % under every line item's medicine name — redundant with the row's own separate Batch column, and cost/margin is sensitive financial info a cashier has no reason to see on every sale. Removed the whole sub-line. 1 new regression test for the links (asserts real `href`s), 1 for the removed cost/margin line, confirmed to fail against the pre-fix code via `git stash`. |
...
| `GET /analytics/summary` and `GET /analytics/daily` have the same dead `Bill.invoice_type == "SALES_RETURN"` bug just fixed in `get_dashboard_analytics` (see the Dashboard row above) | Low | Found in the same pass, same root cause — but confirmed via grep that neither endpoint is called by any real frontend page (both dead/unreferenced), so not fixed now to avoid scope creep on something nothing renders. Fix the same way if either ever gets wired into a real screen: sum real `SalesReturn` rows instead. |
| Billing's 4 money endpoints (`create_bill`, `update_bill`, `create_payment`, `create_refund`) have zero permission check | High — **deliberately deferred to post-launch, Sep 22, 2026** | Confirmed Sep 16, 2026 by `scripts/check_permission_coverage.py` (see RULE MISSES LOG same date) — any authenticated role, cashier included, can call these directly with no role restriction. Left as a reviewed `# permission-exempt:` exception; asked Abinash directly whether to decide the role policy now (e.g. should refunds specifically be tighter than everyday billing) — **decision: leave exactly as-is, do not add ACL here now.** Real usage after launch should drive who gets restricted from what, not a guess made before a single real pharmacy has used it. Revisit post-launch based on actual user feedback, not before. Every other module has the base coverage the automated gate requires (each write is either permission-checked or has a reviewed exemption); Billing remains the one deliberate, named, now-confirmed-intentional exception. |
| 5 mutating endpoints write no audit trail — `inventory.py`'s `create_product`/`update_product`/`delete_product`, `batches.py`'s `update_stock_batch`/`delete_stock_batch` | ✅ **Fixed Sep 22, 2026** | Confirmed Sep 16, 2026 by `scripts/check_audit_log_coverage.py` (see RULE MISSES LOG same date) — a medicine or stock-batch create/edit/delete left no record of who did it, for pharmacy data Manifesto rule 6 calls compliance data. Fixed by adding the same per-router `_record_audit`/`_client_ip` pattern already used by suppliers.py/customers.py/purchases.py/etc. to both files. `batches.py`'s quantity changes keep their existing, more detailed `StockMovement` trail (`_record_movement`) unchanged — the new audit-log call is deliberately scoped to everything else (MRP, cost, expiry, batch number, delete), not a duplicate of it. Real bug caught while building this, not just a doc gap: `ProductORM.gst_rate`/`discount_percent` are `Numeric` columns — SQLAlchemy returns a real `Decimal` at runtime regardless of the model's `Mapped[float]` hint, and a raw `Decimal` isn't JSON-serializable for the `AuditLog.old_values`/`new_values` JSONB columns — a real `gst_percent` edit 500'd until this was cast to `float()` first. Also caught before shipping: the new batch audit entries were first written with `entity_type="batch"`, but the frontend Audit Log page's own `ENTITY_LABELS`/`ENTITY_TYPES` (`AuditLogBadges.tsx`) only recognize `"stock_batch"` — would have silently never matched that page's "Stock Batch" filter. 7 new regression tests (`test_inventory_batch_audit_log.py`), confirmed 5 of 7 fail against the pre-fix code via `git stash`, pass after. |
