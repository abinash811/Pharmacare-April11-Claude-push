...
| As a pharmacist, I want to record a medicine's strength (e.g. 500mg) separately from its name. | ✅ **Fixed August 22, 2026** | `strength` in `ProductCreate`/`ProductUpdate`, Add/Edit Medicine forms, shown inline next to the name on the Inventory table and Medicine Detail header, and now searchable (see search row above). | Not yet a column in the Excel bulk-upload template (`utils/excel.py` doesn't map it, or `dosage_form`, or several other optional fields either — a pre-existing, broader bulk-upload limitation, not new). |
...
| `FilterDrawer.jsx` offers 6 filters; only 2 actually filter anything | 🟡 **Flagged, not fixed this pass** | `useInventorySearch.js`'s `fetchInventory` only reads `activeFilters.stock_status` and `activeFilters.category` into real API params. Dosage Type, GST %, and Location are real dropdowns with real options that silently do nothing when applied — a pharmacist filtering by GST% gets an unfiltered list with no error. Found while wiring the new "Requires Refrigeration only" filter (which *is* fully wired — `cold_chain_only` param, confirmed live). Out of scope for this pass — a separate, pre-existing gap unrelated to strength/refrigeration; flagged here rather than silently left for the next person to rediscover from scratch. |
