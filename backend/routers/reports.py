...
#
# Sep 15, 2026: PharmaCare had per-customer outstanding (Customers page) but
# no consolidated view of every customer who owes money — an owner deciding
# who to call for collection had to open each customer one at a time. This
# is the one-screen answer to "who owes us, how much, since when."
...
async def get_outstanding_dues(
    ...
    rows = (await db.execute(
        select(
            CustomerORM.id, CustomerORM.name, CustomerORM.phone,
            CustomerORM.credit_limit_paise,
            func.sum(BillORM.balance_paise).label("outstanding_paise"),
            func.count(BillORM.id).label("bill_count"),
            func.min(BillORM.bill_date).label("oldest_due_date"),
        )
        .join(BillORM, BillORM.customer_id == CustomerORM.id)
        .where(
            CustomerORM.pharmacy_id == pid, BillORM.pharmacy_id == pid,
            BillORM.status == "due", BillORM.deleted_at.is_(None),
        )
        .group_by(CustomerORM.id, CustomerORM.name, CustomerORM.phone, CustomerORM.credit_limit_paise)
        .order_by(func.sum(BillORM.balance_paise).desc())
    )).all()

    customers = [
        {
            "customer_id": str(cid), "customer_name": name, "customer_phone": phone,
            "credit_limit": _p2r(credit_limit_paise),
            "outstanding": _p2r(outstanding_paise),
            "bill_count": int(bill_count or 0),
            "oldest_due_date": oldest_due_date.isoformat() if oldest_due_date else None,
            "over_limit": bool(credit_limit_paise > 0 and (outstanding_paise or 0) > credit_limit_paise),
        }
        for cid, name, phone, credit_limit_paise, outstanding_paise, bill_count, oldest_due_date in rows
    ]
    total_outstanding_paise = sum((row.outstanding_paise or 0) for row in rows)

    return {
        "summary": {
            "total_outstanding": _p2r(total_outstanding_paise),
            "customer_count": len(customers),
            "bill_count": sum(c["bill_count"] for c in customers),
        },
        "customers": customers,
    }
