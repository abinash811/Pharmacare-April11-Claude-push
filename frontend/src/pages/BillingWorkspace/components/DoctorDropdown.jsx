...
export default function DoctorDropdown({ value = '', onChange, readOnly = false }) {
  ...
  return (
    <div className="relative" ref={containerRef}>
      ...
      >
        <Stethoscope className="w-4 h-4 text-slate-400" />
        <span className={`text-sm font-medium truncate max-w-[100px] ${value ? 'text-slate-900' : 'text-slate-400'}`}>
          {value || 'Doctor'}
        </span>
        ...
        <div className="absolute top-full left-0 mt-1 bg-white border border-slate-200 rounded-lg shadow-xl z-50 w-64 max-h-56 overflow-hidden">
          ...
          <div className="p-2 border-b border-slate-100">
            <input
              ...
              autoFocus
              className="w-full px-3 py-1.5 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#4682B4]"
