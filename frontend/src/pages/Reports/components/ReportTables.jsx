...
import { FileText, Package, Clock, TrendingUp, Undo2, LineChart } from 'lucide-react';
    ...
    <table className="w-full" data-testid="expiry-report-table">
      ...
      <tbody className="divide-y">
        {!data?.length ? (
          ...
          <tr key={idx} className={`hover:bg-brand-tint transition-colors ${row.days_to_expiry < 0 ? 'bg-red-50' : ''}`}>
...
// ── Price Variation ───────────────────────────────────────────────────────────
function PriceChangeBadge({ change, percent }) {
...
function PriceVariationTable({ data }) {
  return (
