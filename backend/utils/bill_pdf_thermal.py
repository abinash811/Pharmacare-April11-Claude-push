...
Mirrors PrintReceipt.jsx's ThermalReceipt field set and layout exactly
(name/qty/amount item rows, no GST breakdown table, no signature line) so
a reprint/download from here looks the same as the original in-session
"Save & Print", not a differently-formatted document.
"""
