...
const getErrorMessage = (error: unknown, fallback = 'Something went wrong'): string => {
  ...
  const e = error as { message?: string; response?: { data?: { detail?: unknown } } };
...
export const useApiCall = () => {
  ...
  const execute = useCallback(async <T>(apiFn: () => Promise<{ data: T }>, options: {
    successMsg?: string;
    errorMsg?: string;
    showToast?: boolean;
    transform?: (res: { data: T }) => T;
  } = {}): Promise<T | null> => {
    const {
      successMsg = '',
 ...
 */
export const useFetch = <T>(apiFn: (() => Promise<{ data: T }>) | null, deps: unknown[] = [], options: {
  errorMsg?: string;
  initialData?: T | null;
  transform?: (res: { data: T }) => T;
  skip?: boolean;
...
const useParallelFetch = (apiFns: Array<() => Promise<any>> = [], deps: unknown[] = [], options: {
  errorMsg?: string;
  transforms?: Array<(res: any) => any>;
  skip?: boolean;
} = {}) => {
  const {
    errorMsg   = 'Failed to load data',
