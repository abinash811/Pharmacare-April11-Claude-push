# PharmaCare — Roadmap
# Version: 2.64 | Last updated: September 13, 2026
# Type: Living Status
...
**New finding, Sep 13, 2026 (Inventory product-review) — the Low Stock / Near Expiry toggles in `NotificationsTab.tsx` don't do anything real, anywhere:**
- The tab's own copy says "These alerts appear as in-app notifications when conditions are met" and shows a live `ToastPreview` mockup per card — but `ToastPreview` is a static illustration with hardcoded example text, not wired to any real condition. Grepped the whole frontend: no code anywhere generates a real toast when stock is actually low or a batch is actually near expiry.
- `GET /analytics/dashboard` does correctly compute and return `alerts_config: {low_stock_enabled, near_expiry_enabled}` from the real `PharmacySettings` columns these toggles write to — but `Dashboard/index.jsx` never reads `alerts_config` from the response, and `AlertsPanel.jsx` (the Low Stock / Expiring Soon cards) has no gating logic at all — it renders unconditionally from whatever `low_stock`/`expiring_soon` arrays the API returns, regardless of either toggle.
- Net effect: turning either toggle off in Settings has **zero effect anywhere in the app** — not on a toast (none exists) and not on the one on-screen surface (Dashboard's AlertsPanel) that already has the right data to gate on. Confirmed by code trace, not yet fixed — this is a product/scope decision (build the real toast-firing mechanism the copy promises, vs. scope the toggle down to gate the Dashboard panel it's closest to) rather than a one-line fix, so it's logged here for a decision rather than silently patched.
...
- **v1 (fix, not a new feature):** the Notifications toggle finding above — either build the real toast-firing mechanism the Settings copy promises, or scope the toggle down to gate the Dashboard AlertsPanel it's positioned nearest to. Needs a decision, not a guess — see the finding above.
- **v2:** tiered expiry warnings (30/60/90-day escalation, Marg-validated) — a real refinement, but the single near-expiry threshold already works and is enforced; return-to-supplier discoverable from the Inventory health screen / near-expiry alert itself (the capability exists via Purchases, this is a discoverability gap); default/preferred supplier link on a product (real but not urgent).
