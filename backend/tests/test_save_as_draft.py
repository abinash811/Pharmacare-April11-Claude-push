...
class TestSaveAsDraftFeature:
    ...
    def setup(self):
        ...
        products_response = requests.get(
            f"{BASE_URL}/api/products/search-with-batches?q=para",
    ...
    def test_draft_bill_gets_draft_prefix_number(self):
        ...
        assert bill['bill_number'].startswith(
            'DRAFT-'), f"Draft bill should have DRAFT- prefix, got: {bill['bill_number']}"
    ...
    def test_draft_does_not_deduct_stock(self):
        ...
        assert stock_after_draft == initial_stock, (
            f"Stock should NOT be deducted for draft. "
            f"Initial: {initial_stock}, After draft: {stock_after_draft}")

        print(
            f"✓ Draft bill did NOT deduct stock (Initial: {initial_stock}, After: {stock_after_draft})")
    ...
    def test_regular_save_deducts_stock(self):
        ...
        assert bill['bill_number'].startswith(
            'INV-'), f"Paid bill should have INV- prefix, got: {bill['bill_number']}"
        ...
        assert stock_after_paid < initial_stock, (
            ...
            f"Initial: {initial_stock}, After: {stock_after_paid}")

        print(
            f"✓ Paid bill DID deduct stock (Initial: {initial_stock}, After: {stock_after_paid}, "
    ...
    def test_regular_save_gets_inv_prefix(self):
        ...
        assert bill['bill_number'].startswith(
            'INV-'), f"Paid bill should have INV- prefix, got: {bill['bill_number']}"
        assert not bill['bill_number'].startswith(
            'DRAFT-'), "Paid bill should NOT have DRAFT- prefix"
    ...
    def test_draft_bill_response_structure(self):
        ...
        assert bill['bill_number'].startswith(
            'DRAFT-'), f"Bill number should start with DRAFT-, got: {bill['bill_number']}"

        print("✓ Draft bill response structure is correct")
