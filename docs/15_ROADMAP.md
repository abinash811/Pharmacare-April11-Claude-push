# PharmaCare — Roadmap
# Version: 2.99 | Last updated: September 18, 2026
# Type: Living Status
...
| Same-day edit for a finalized (paid/due) bill | 📋 **Decided Sep 18, 2026, not yet built** | Today `PUT /bills/{id}` hard-blocks any edit once `status != "draft"` — the only fix for a mistaken finalized bill is a Sales Return (see `docs/07_BUSINESS_LOGIC.md`'s "WHAT CANNOT BE DONE" table). Direct decision after discussing why competitors lock finalized bills (GST law forbids silently altering an issued invoice; stock is already deducted; unrestricted edits break the audit trail) — landed on a middle ground rather than either full lock-forever or free editing: allow editing a paid/due bill **only until that day's Day-End Closing is run**, under three conditions, all reusing patterns already built for Sales Return's financial-edit path: (1) old vs new values recorded via the existing audit-log mechanism, not a silent overwrite; (2) stock reversed-then-reapplied against the real batches, not just the numbers changed; (3) blocked outright if a Sales Return already exists against that bill (avoids a return and an edit fighting over the same line items), and blocked once Day-End Closing has been run for that bill's date. Not yet implemented — `update_bill`'s `status == "draft"` check is still the only gate in code today. |
