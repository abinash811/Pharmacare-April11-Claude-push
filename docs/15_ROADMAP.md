# PharmaCare — Roadmap
# Version: 2.81 | Last updated: September 15, 2026
# Type: Living Status
...
| Credit / due bills | 🚫 **Creation blocked Sep 14, 2026 — direct product decision** | New due/partial-payment bills can no longer be created — every new bill must be paid in full at checkout. `create_bill` and `update_bill` (the two real entry points that could produce a "due" bill) now both reject with HTTP 400 before any Bill row/invoice number/stock deduction. `_check_credit_limit` (billing.py) removed as now-fully-dead code — `Customer.credit_limit_paise` and its Customers-page UI are untouched (still informational for any pre-existing due balance). Frontend: "Credit" removed from `BillingSubbar.jsx`'s PAYMENT_TYPES. Pre-existing due bills (created before this change) are NOT affected — see the next row and "Due" filter/Pending Payments card, all still fully functional for handling them. |
| Record payment on due bill | ⚠️ **Still backend-only, no UI path — unchanged by the Sep 14, 2026 decision above** | The open question this row previously tracked ("should due bills be allowed at all?") is now answered — yes, for any pre-existing due bill, since only NEW due-bill creation is blocked. `POST /api/payments` still works and is still the intended way to collect on a legacy due bill. The actual UI gap from Sep 13 is unchanged and still real: `BillingHeader`'s "Collect Payment" button is still wired to a placeholder stub (`toast.info('Collect payment coming soon')`, `BillingWorkspace/index.jsx`) — verified still true Sep 14, 2026. Building the real collection screen is separate work, not done in this change. |
