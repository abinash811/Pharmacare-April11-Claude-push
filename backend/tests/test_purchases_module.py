...
class TestPurchasesModule:
    ...
    def test_get_purchases_with_filters(self):
        ...
        response = self.session.get(
            f"{BASE_URL}/api/purchases?supplier_id={supplier['id']}&page_size=10")
    ...
    def test_create_confirmed_credit_purchase(self):
        ...
        assert actual_increase == expected_increase, (
            f"Supplier outstanding should increase by {expected_increase}, got {actual_increase}")
    ...
    def test_confirmed_purchase_creates_stock_batch(self):
        ...
        batches_response = self.session.get(
            f"{BASE_URL}/api/stock-batches?product_sku={product['sku']}")
        ...
        if batches_response.status_code == 200:
            ...
            if our_batch:
                ...
                print(
                    f"✓ Stock batch created with PTR={ptr_value}, priority={batch_priority}, qty=35")
            else:
    ...
    def test_mark_purchase_as_paid(self):
        ...
        supplier_before = self.session.get(
            f"{BASE_URL}/api/suppliers/{purchase['supplier_id']}").json()
        outstanding_before = supplier_before.get('outstanding', 0)
        ...
        supplier_after = self.session.get(
            f"{BASE_URL}/api/suppliers/{purchase['supplier_id']}").json()
        outstanding_after = supplier_after.get('outstanding', 0)
    ...
    def test_partial_payment(self):
        ...
        assert updated_purchase[
            'amount_paid'] == partial_amount, f"Amount paid should be {partial_amount}"
    ...
    def test_auto_due_date_calculation(self):
        ...
        purchase = response.json()
        assert purchase.get(
            'due_date') is not None, "Due date should be auto-calculated for credit purchase"
