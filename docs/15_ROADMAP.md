# PharmaCare — Roadmap
# Version: 2.90 | Last updated: September 15, 2026
# Type: Living Status
...
- **v1:** the "Multi" payment button — needs a decision (remove it vs. build a real split-entry UI), same shape as the Sep 13 Inventory Notifications-toggle decision. ~~a day-end closing/Z-report~~ — built Sep 15, 2026, see row above. ~~a real Sales Returns entry point + due-balance credit~~ — built Sep 15, 2026, see row above. ~~manual sales returns (no original bill)~~ — built Sep 15, 2026, see `docs/07_BUSINESS_LOGIC.md` FLOW 2; `get_product_transactions`'s inner join on `original_bill_id` (flagged below) fixed in the same change.
...
| Receipt & Print settings — Print sub-tab | ✅ **Real bug found + fixed Sep 14, 2026 — live browser test** | Live bill preview, paper size (A4/A5/58mm/80mm), show/hide toggles, header/footer, all persist correctly and are consumed by the backend PDF generator (`routers/billing.py`) — **but** the real frontend print output (`BillingWorkspace/components/PrintReceipt.jsx`, a separate component from Settings' own `BillPreview.tsx` mock) never rendered `bill_header` on the 80mm/58mm thermal format (the default paper size) — `ThermalReceipt` destructured `bill_footer` but not `bill_header`, so a pharmacist's custom header text silently never reached the counter printer while the footer worked fine. `A4Invoice` was already correct. Confirmed live: registered a zero-data pharmacy, set a custom header via the real Settings UI, created and printed a real bill via the real Billing UI, captured the actual print-only DOM (`window.print` override, since headless Chromium doesn't block on the print dialog) — header was missing before the fix, present after. Fixed by adding `bill_header` to `ThermalReceipt`'s destructure + render, same placement as `A4Invoice`. New `PrintReceipt.test.tsx` (5 tests, all 4 paper formats) locks this in — this component had zero test coverage before. See RULE MISSES LOG for why the Sep 13 pass's "real end-to-end" claim didn't catch this. |
| Receipt & Print settings — Digital sub-tab | ⚠️ **Decorative** — found Sep 13, 2026 | `use_default_header`/header-footer image/text fields save and read back correctly, but nothing in the backend (no WhatsApp/email/download renderer) ever reads `digital_*` fields — pure round-trip to nowhere. Not fixed — no digital-receipt renderer exists yet to wire it into; flagged, not silently patched. |
...
|------|---------------|----------------------|--------------|
| Sep 15, 2026 | Manifesto rule 10 ("every error notification must say why") — 4 catch blocks in `PurchaseReturnCreate/index.jsx` (lines 66, 118) and `PurchaseReturnDetail/index.jsx` (lines 52, 63) read `err.response?.data?.detail` directly instead of the normalized `err.message` | Tooling gap: found during a `product-review` of Purchase Returns, not this session's own new code — `scripts/check_error_messages.py` (design-guard Rule 14) only flags a `toast.error(...)` that doesn't reference the caught error at all; it has no way to tell that a call *does* reference the error but reads an unsafe raw field (`.response.data.detail`, which FastAPI returns as an array-of-objects for a 422, not a string) instead of the safe, pre-normalized `.message`. The identical root cause (reading raw `.detail` instead of `.message`) had already been found and fixed the same day in `SalesReturnCreate/index.jsx` during the manual-returns build, live, via a real 422 — that fix was never generalized into a repo-wide grep for the same pattern in sibling return flows. Not fixed in this review pass (review-only, no build requested yet) — flagged for a build decision. | None yet — logged for the next Purchase Returns build pass. When fixed: same pattern as `SalesReturnCreate`'s fix (`toast.error(err.message \|\| 'fallback')`). **Gate to close**: extend `check_error_messages.py` to also flag `err.response?.data?.detail`/`err.response.data.detail` reads directly in a `toast.error(...)` call (not just a missing `.message` reference) — a mechanical, low-false-positive pattern match, unlike the blob-response-type gap below which genuinely can't be statically detected. |
