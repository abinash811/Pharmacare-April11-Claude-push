# PharmaCare — Roadmap
# Version: 2.88 | Last updated: September 15, 2026
# Type: Living Status
...
- **v1:** the "Multi" payment button — needs a decision (remove it vs. build a real split-entry UI), same shape as the Sep 13 Inventory Notifications-toggle decision. ~~a day-end closing/Z-report~~ — built Sep 15, 2026, see row above. ~~a real Sales Returns entry point + due-balance credit~~ — built Sep 15, 2026, see row above.
- **v2:** the same "type a name with no match → Add [name]" inline flow `PatientCombobox` already has, applied to Doctor (today's free-text field works but isn't discoverable the same way); WhatsApp — attach the actual bill (PDF/image), not just a text summary, plus a due-payment reminder message (Marg-validated); a real payment-gateway (UPI QR/card) integration reconciled automatically against the bill; **manual sales returns (no original bill)** — the Settings toggle and permission exist, but `create_sales_return` unconditionally 400s without a bill ID regardless, and the frontend has no bill-search UI to reach it either — a real, named Marg-validated gap (bill-by-bill credit-note adjustment), just not wired end-to-end on either side; once built, also fix `get_product_transactions` (inventory.py)'s inner join on `SalesReturn.original_bill_id`, which would silently drop a manual return (no original bill) from a medicine's transaction history.
